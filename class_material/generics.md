...
Now, since both the get() and add() methods of the class List work with, and return, object of the class Object (which is the super class of every class in Java), we can never be sure, at compilation time, what is the type of o. The reason why the interface of the List collection operate over Objects is to make them as reusable as possible: we can make a List of any object. The cost is that we lose in type safety: we can now have Lists of apples and oranges, the compiler will not help us check what we intend: we would like a List of only apples or only oranges.
...
For more info on generics, and how to write your own generic class, see this Java Technical Article on Generics and this Generics tutorial.
