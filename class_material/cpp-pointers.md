...
Space for a primitive of type int * is allocated on the activation frame of the main function.
The space allocated is associated with the variable i_ptr.
Space for a primitive of type int is allocated on the heap (using the new operator), and is initialized to 10.
The address of the newly allocated integer is saved in i_ptr.
The operator << of std::cout is passed the content (by value) i_ptr points to. That is, std::cout's operator << is called with the integer 10.
...
Space for a Cow is allocated on the activation frame of the main function, and the constructor of Cow is called with 482528404, while this points to the address of the space allocated on the stack.
The space allocated is associated with the variable bety.
Space for a pointer Cow * is allocated on the activation frame of the main function.
The space allocated is associated with the variable ula.
Space for a Cow is allocated on the heap (using the new operator), and its constructor is called with 834579343, while this points to the address of the space allocated on the heap.
The address of the newly allocated Cow is saved in ula.
...
const pointers
Any type in C++ can be marked as const, which means that its value cannot be changed (or even reassigned). A const pointer is declared by adding the const keyword after the type, for example, int *const i_ptr is a const pointer to an int
...
Actually, there are only two ways of passing parameters to function; by value and by reference. By pointer is actually by value, where the value being copied is an address.
...
By pointer, for either 'in' or 'out' parameters.
...
Arrays are mainly needed when you have to call some legacy C code, which works with plain old arrays. However, even in these cases, it is possible to convert a Vector to an array before calling the C code.
Let us start with a word of caution: unless you really need to, do not use arrays in C++. They are dangerous. Use Vectors instead. We will see some reasons why later.
