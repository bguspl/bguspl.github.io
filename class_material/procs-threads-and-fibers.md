...
[^register]: A register is a small cell of very fast memory located physically inside the CPU. Registers are the fastest type of memory, in contract to the slower RAM main memory, located outside of the CPU.
...
* Processor state information: contents of registers[^register], Program Counter (PC), Stack Pointer
...
**Scheduling Paradigms**: There are two paradigms with which the scheduler may interact with the currently active processes:
...
[^ContextSwitchAndThreads]: When performing a context switch between threads in the same process, not all steps of a regular context switch need to be taken. For example, there is no need to restore any context of the process (but just to switch stacks) and no need to flush the CPU cache, as threads in the same process share their memory.
...
In contrast to different processes, threads inside the same process share the same memory space (however, each thread has its own stack), the same opened files and access rights. Moreover, the cost of a context switch between threads in the same process is much lower than between processes or threads of different processes, as the CPU cache need not be flushed[^ContextSwitchAndThreads].
...
```
you can read more about the interrupt flag [here](https://docs.oracle.com/javase/tutorial/essential/concurrency/interrupt.html).
...
![image](../images/seq-par-expr2.png)
